...
module.exports = (req, res) => {
  ...
  const asJson =
    parsedUrl.searchParams.get("json") === "true" ||
  ...
  const proxyImageUrl = `/api/images/${encodeURIComponent(item.filename)}`;
  const rawImageUrl = `${rawBaseUrl}${encodeURIComponent(item.filename)}`;
  ...
  // Default: 302 Found redirect to proxy image
  res.writeHead(302, {
    Location: proxyImageUrl,

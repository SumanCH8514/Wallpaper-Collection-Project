/**
 * Robust High-Performance Image Proxy
 * Endpoint: /api/images/:file or /api/images?file=:file
 */
...
module.exports = (req, res) => {
  ...
  // Extract filename from query parameter or path segment
  ...
  if (!validFilenames.has(safeFilename.toLowerCase())) {
    ...
    return res.end(
      JSON.stringify({ error: "Image not found in collection" }),
    );
  }
  ...
  https
    .get(targetUrl, options, (proxyRes) => {
      if (proxyRes.statusCode !== 200) {
        ...
        return res.end(
          JSON.stringify({
            error: `Failed to fetch image: HTTP ${proxyRes.statusCode}`,
          }),
        );
      }

      // Forward content headers & set permanent Edge CDN cache headers
      res.setHeader("Content-Type", contentType);
      res.setHeader(
        "Content-Disposition",
    ...
    .on("error", (err) => {
      ...
      res.end(
        JSON.stringify({ error: "Bad gateway connecting to upstream source" }),
      );
    });
};
